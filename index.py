...
pygame.display.set_caption("Breaking Bricks - By Python [Sourav Barua]")
...
class Brick:
    ...
    def hit(self):
        ...
        self.color = self.interpolate(
            (255, 255, 255), (0, 0, 0), self.health/self.max_health)
...
def generate_bricks(rows, cols, colors):
    gap = 2
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            brick = Brick(col * brick_width + gap * col, row * brick_height +
                          gap * row, brick_width, brick_height, 2, color)
            bricks.append(brick)
...
def game_start_screen():
    ...
    while not start:
        win.fill((255, 255, 255))
        start_text = LIVES_FONT.render("Press SPACE to Start", 1, (0, 0, 0))
...
def main():
    ...
    bricks = generate_bricks(3, 10, brick_colors)
    lives = 3
    ...
    def display_text(text):
        text_render = LIVES_FONT.render(text, 1, (255, 0, 0))
